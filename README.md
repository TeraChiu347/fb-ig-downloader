...
| 版本 | 檔案 | 說明 |
|------|------|------|
| 桌面版 | `main.py` | tkinter 原生視窗，每個人在自己電腦上執行 |
| 本機網頁版 | `web_app.py` + `templates/index.html` | 本機網頁，雙擊開啟後自動跳出瀏覽器，有貓咪跑跑進度動畫，每個人在自己電腦上執行 |
| **雲端網頁版**（給特定朋友用） | `server.py` + `templates/server_index.html` | 部署到 Render，朋友打開網址就能用，不用下載任何東西，見下方「部署到 Render」 |

前兩個版本是「每個人在自己電腦上執行」；雲端網頁版是「架在網路上，開連結就能用」，架構完全不同——伺服器不能寫入訪客的本機 `~/Downloads`，所以改成下載完成後在網頁上出現一個「⬇️ 下載」連結，由訪客的瀏覽器自己存檔。
...
## 部署到 Render（雲端網頁版，給特定朋友用）

[Render](https://render.com) 有真正的免費方案，直接接 GitHub repo 就能部署，不需要信用卡，適合小範圍朋友共用。

### 部署步驟

1. 到 [render.com](https://render.com) 用 GitHub 帳號註冊/登入
2. 右上角 **New +** → **Blueprint**
3. 選擇 `TeraChiu347/fb-ig-downloader` 這個 repo（Render 會自動讀取專案裡的 `render.yaml`，不用手動填 build/start command）
4. 按 **Apply**，等它 build 完（第一次大約 2-3 分鐘）
5. 完成後會拿到一個網址，例如 `https://fb-ig-downloader-xxxx.onrender.com`，把這個網址分享給朋友即可

### 使用限制（免費方案）

- **會休眠**：閒置約 15 分鐘後會自動休眠，下次有人打開網址時要重新啟動，第一次連線可能要等 30-50 秒才會有反應，之後就正常快
- **沒有密碼保護**：任何拿到網址的人都能用，目前設計上刻意不加密碼（照需求），如果連結外流出去、被不認識的人拿去用，無法阻擋，之後要加可以再說
- **檔案不會永久保留**：下載連結只能用一次，且 30 分鐘沒點擊會自動清除，避免免費方案的暫存空間塞滿
- **只能開一個 worker**（`render.yaml` 裡已設定 `--workers 1`）：因為下載狀態存在程式記憶體裡，多個 worker 會互相看不到彼此的檔案，**不要為了效能自己改成多 worker**
- 流量/頻寬如果用量變大，可能會碰到 Render 免費方案的流量上限，屆時要嘛升級付費方案、要嘛減少使用人數

### 之後要更新怎麼辦

Render 有接 GitHub，`git push` 到 `master` 分支後會自動重新部署，不用手動操作。
